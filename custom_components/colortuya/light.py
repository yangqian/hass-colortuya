"""
Simple platform to control LOCALLY Tuya RGB devices.
...
class TuyaCache:
    ...
    def set_color(self, hue,saturation,brightness):
        log.warn(
            "h:{0},s{1},v{2}".format(hue,saturation,brightness))
        r,g,b=colorutil.color_hsv_to_RGB(hue,saturation,brightness*100/255)
        log.warn(
            "r:{0},g{1},b{2}".format(r,g,b))
        for _ in range(UPDATE_RETRY_LIMIT):
            try:
                return self._device.set_colour(int(r),int(g),int(b))
            except ConnectionError:
                pass
            except socket.timeout:
                pass
        log.warn(
            "Failed to set color after {} tries".format(UPDATE_RETRY_LIMIT))
    def state(self):
        self._device.state()
...
class TuyaDevice(LightEntity):
    ...
    def turn_on(self, **kwargs):
        ...
        if ATTR_BRIGHTNESS in kwargs:
          ...
          if self._colormode:
            hue,saturation=self._color
            self._device.set_color(hue,saturation,converted_brightness)
          else:
            if 0 < converted_brightness <= 25:
        ...
        if ATTR_HS_COLOR in kwargs:
            ...
            self._device.set_color(hue,saturation,brightness)
            self._color=hue,saturation
